...
namespace ZAP.CRM.Catalog.Infrastructure.Repositories.Brands;

public class BrandRepository : ZAP.Ecosystem.Shared.Data.BaseRepository<Brand>, IBrandRepository
{
    private readonly EcosystemDbContext _context;
    public BrandRepository(EcosystemDbContext context) : base(context)
    {
        _context = context;
    }

    public async Task<IEnumerable<Brand>> GetAllAsync(Guid? tenantId = null) 
        => await _context.Set<Brand>().Where(x => tenantId == null || x.tenant_id == tenantId).ToListAsync();

    public async Task<(IEnumerable<Brand> Items, int Total)> GetPagedAsync(
        int page, int pageSize, string? search = null, Guid? tenantId = null, int? statusId = null, string sortField = "name", bool sortDescending = false)
    {
